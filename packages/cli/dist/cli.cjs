...
var ir = require('@fir/ir');
var schema = require('@fir/ir/schema.json');
var compat = require('@fir/compat');
var runtimeCore = require('@fir/runtime-core');
var adapterVoiceSsml = require('@fir/adapter-voice-ssml');
...
async function serve(irPath, options = {}) {
  ...
  if (!fs__namespace.existsSync(irPath)) {
    throw new Error(`IR file not found: ${irPath}`);
  }
  const irData = JSON.parse(fs__namespace.readFileSync(irPath, "utf8"));
  const indexHtml = `
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>FIR Spatial Preview - ${path__namespace.basename(irPath)}</title>
  <style>
    body {
      margin: 0;
    ...
    switch (req.url) {
      ...
      case "/runtime-core.js":
        try {
          ...
          const content = fs__namespace.readFileSync(runtimeCorePath, "utf8");
          res.writeHead(200, {
      ...
      case "/adapter-web-dom.js":
        try {
          ...
          const content = fs__namespace.readFileSync(adapterPath, "utf8");
          res.writeHead(200, {
            "Content-Type": "application/javascript",
            "Cache-Control": "no-cache"
          });
      ...
      case "/spatial-kernel.js":
        try {
          const kernelPath = path__namespace.join(
            path__namespace.dirname(require2.resolve("@fir/spatial-kernel")),
            "index.js"
          );
          const content = fs__namespace.readFileSync(kernelPath, "utf8");
          res.writeHead(200, {
            "Content-Type": "application/javascript",
            "Cache-Control": "no-cache"
          });
...
program.command("validate").argument("<input>").action((input) => {
  const ir$1 = ir.loadIR(input);
  const res = ir.validateIR(ir$1, schema__default.default);
  if (!res.ok) {
...
program.command("prove").argument("<input>").action((input) => {
  ...
  const { messages } = compat.lintCompat(ir$1);
  ...
  console.log("PROOF OK. ir_hash:", ir.irHash(ir$1));
});
program.command("render").option("--adapter <name>", "voice").argument("<input>").action((input, opts) => {
  const ir$1 = ir.loadIR(input);
  const tree = runtimeCore.buildSemanticTree(ir$1, { scale: 1 });
  if (opts.adapter === "voice") {
    console.log(adapterVoiceSsml.renderToVoice(tree));
  } else {
