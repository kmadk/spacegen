...
export class AIPatternAnalyzer {
  ...
  constructor(config: AIAnalysisConfig) {
    this.config = {
      ...
      maxTokens: parseInt(process.env.OPENAI_MAX_TOKENS || "8000"), // Increased for GPT-5
  ...
  async analyzeCostOptimized(
    ...
    try {
      ...
      console.log(`💰 Using ${model.name} for batch analysis (${estimatedTokens} tokens, ~$${this.costOptimizer.estimateCost(model.name, estimatedTokens).toFixed(4)})`);
      ...
      console.warn('💸 Cost-optimized analysis failed, falling back:', error);
      return this.analyzeWithRules(designData);
    }
  }
  ...
  async analyzeDesignPatternsWithVision(
    ...
    if (this.config.debug) {
      console.log("🤖 Starting enhanced AI analysis with vision support...", {
  ...
  async analyzeDesignPatterns(designData: DesignData): Promise<{
    ...
    if (this.config.debug) {
      console.log("🤖 Starting AI analysis of design patterns...");
    }
