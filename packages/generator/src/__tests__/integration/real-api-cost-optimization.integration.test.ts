 ...
 * These tests run only when OPEN_AI_API_KEY is provided
 */
...
const shouldRunRealAPITests =
  !!process.env.OPEN_AI_API_KEY || !!process.env.OPEN_AI_API_KEY;

describe.skipIf(!shouldRunRealAPITests)(
  "Real API Cost Optimization Tests",
  () => {
    ...
    beforeAll(() => {
      if (!shouldRunRealAPITests) {
        console.log(
          "⏭️  Skipping real API tests - no OPEN_AI_API_KEY provided"
        );
        return;
      }
    ...
    it("should perform real cost-optimized batch analysis", async () => {
      ...
      console.log(
        `📊 Results: ${result.entities.entities.length} entities, ${result.endpoints.endpoints.length} endpoints`
      );

      // Log entities for verification
      result.entities.entities.forEach((entity) => {
        console.log(
          `  📋 Entity: ${entity.name} (${entity.fields?.length || 0} fields)`
        );
      });
    ...
    it("should demonstrate caching with real API", async () => {
      ...
      expect(result1.entities.entities.length).toBe(
        result2.entities.entities.length
      );
      expect(result1.endpoints.endpoints.length).toBe(
        result2.endpoints.endpoints.length
      );
      ...
      console.log(
        `⚡ Cache speedup: ${Math.round(duration1 / duration2)}x faster`
      );
    ...
    it("should generate complete backend using cost-optimized analysis", async () => {
      ...
      console.log(
        `📁 Generated: ${result.models.length} models, ${result.endpoints.length} endpoints, ${result.files.length} files`
      );

      // Log generated models
      result.models.forEach((model) => {
        console.log(
          `  🗃️  Model: ${model.name} (${model.fields?.length || 0} fields)`
        );
      });
    ...
    it("should compare costs between old and new methods", async () => {
      ...
      console.log(
        `📊 Result quality: ${optimizedResult.entities.entities.length} entities detected`
      );
